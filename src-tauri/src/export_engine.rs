...
impl ExportEngine {
    ...
    pub fn export_to_docx(&self, data: &UserDataExport, output_path: &Path) -> Result<()> {
        ...
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("BEAR AI - Data Export Report").size(32).bold())
                .align(AlignmentType::Center)
        );

        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text(&format!("Export Date: {}", data.export_date.format("%Y-%m-%d %H:%M:%S UTC"))).size(24))
                .align(AlignmentType::Center)
        );
        ...
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("GDPR Article 20 Compliance Statement").size(28).bold())
        );
        ...
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("Export Metadata").size(28).bold())
        );

        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text(&format!("Version: {}", data.version)).size(22))
        );

        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text(&format!("User ID: {}", data.user_id)).size(22))
        );

        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text(&format!("Integrity Hash: {}", data.metadata.export_hash)).size(22))
        );
        ...
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("Chat History").size(28).bold())
        );

        for (idx, chat) in data.chats.iter().enumerate() {
            docx = docx.add_paragraph(
                Paragraph::new()
                    .add_run(Run::new().add_text(&format!("{}. {}", idx + 1, chat.title)).size(24).bold())
            );

            docx = docx.add_paragraph(
                Paragraph::new()
                    .add_run(Run::new().add_text(&format!("Created: {} | Model: {}",
            ...
            for msg in &chat.messages {
                ...
                docx = docx.add_paragraph(
                    Paragraph::new()
                        .add_run(Run::new().add_text(&format!("[{}] ", role_label)).size(22).bold())
        ...
        docx = docx.add_paragraph(
            Paragraph::new()
                .add_run(Run::new().add_text("Processed Documents").size(28).bold())
        );

        for doc in &data.documents {
            docx = docx.add_paragraph(
                Paragraph::new()
                    .add_run(Run::new().add_text(&format!("• {}", doc.filename)).size(22).bold())
            );
    ...
    pub fn export_to_markdown(&self, data: &UserDataExport, output_path: &Path) -> Result<()> {
        ...
        markdown.push_str(&format!("- **Integrity Hash:** `{}`\n", data.metadata.export_hash));
        markdown.push_str(&format!("- **GDPR Article 20 Compliant:** {}\n", if data.metadata.compliance_info.gdpr_article_20 { "✓" } else { "✗" }));
        markdown.push_str(&format!("- **Encrypted:** {}\n", if data.metadata.compliance_info.encrypted { "✓" } else { "✗" }));
        markdown.push_str(&format!("- **Integrity Verified:** {}\n\n", if data.metadata.compliance_info.integrity_verified { "✓" } else { "✗" }));
        markdown.push_str("---\n\n");
        ...
        for (idx, chat) in data.chats.iter().enumerate() {
            ...
            markdown.push_str(&format!("**Created:** {} | **Model:** {} | **Messages:** {}\n\n",
                chat.created_at.format("%Y-%m-%d %H:%M:%S"),
                chat.model_used,
                chat.messages.len()
            ));
        ...
        for doc in &data.documents {
            ...
            markdown.push_str(&format!("- **Upload Date:** {}\n", doc.upload_date.format("%Y-%m-%d")));
            markdown.push_str(&format!("- **Chunks:** {}\n", doc.chunk_count));
            markdown.push_str(&format!("- **PII Detections:** {}\n\n", doc.pii_detections.len()));

            if !doc.pii_detections.is_empty() {
                markdown.push_str("**PII Detections:**\n\n");
                for pii in &doc.pii_detections {
                    markdown.push_str(&format!("- Type: {} | Confidence: {:.2}% | Replaced with: {}\n",
                        pii.pii_type,
        ...
        // Footer
        markdown.push_str("---\n\n");
        markdown.push_str("*This export was generated by BEAR AI in compliance with GDPR Article 20.*\n");
    ...
    pub fn export_to_pdf(&self, data: &UserDataExport, output_path: &Path) -> Result<()> {
        let (doc, page1, layer1) = PdfDocument::new("BEAR AI Data Export", Mm(210.0), Mm(297.0), "Layer 1");
        ...
        // Title
        current_layer.use_text("BEAR AI - Data Export Report", 24.0, Mm(50.0), Mm(270.0), &font_bold);
        current_layer.use_text(
            &format!("Export Date: {}", data.export_date.format("%Y-%m-%d %H:%M:%S UTC")),
            12.0,
            Mm(50.0),
            Mm(260.0),
            &font
        ...
        // Compliance Statement
        current_layer.use_text("GDPR Article 20 Compliance Statement", 14.0, Mm(20.0), Mm(240.0), &font_bold);
        current_layer.use_text(
        ...
        current_layer.use_text(&format!("Version: {}", data.version), 10.0, Mm(20.0), Mm(200.0), &font);
        current_layer.use_text(&format!("User ID: {}", data.user_id), 10.0, Mm(20.0), Mm(195.0), &font);
        current_layer.use_text(&format!("Hash: {}", &data.metadata.export_hash[..16]), 10.0, Mm(20.0), Mm(190.0), &font);

        // Chat History Summary
        let mut y_pos = 170.0;
        current_layer.use_text("Chat History Summary", 14.0, Mm(20.0), Mm(y_pos), &font_bold);
        y_pos -= 10.0;
        ...
        for (idx, chat) in data.chats.iter().enumerate() {
            ...
            current_layer.use_text(
                &format!("{}. {} ({} messages)", idx + 1, chat.title, chat.messages.len()),
                10.0,
                Mm(20.0),
                Mm(y_pos),
                &font
    ...
    pub fn export_to_text(&self, data: &UserDataExport, output_path: &Path) -> Result<()> {
        ...
        text.push_str("-".repeat(80).as_str());
        text.push_str("\n");
        text.push_str("This export has been generated in accordance with the General Data Protection\n");
        text.push_str("Regulation (GDPR) Article 20, providing you with the right to data portability.\n\n");
        ...
        text.push_str(&format!("GDPR Compliant: {}\n", if data.metadata.compliance_info.gdpr_article_20 { "Yes" } else { "No" }));
        text.push_str(&format!("Encrypted: {}\n", if data.metadata.compliance_info.encrypted { "Yes" } else { "No" }));
        text.push_str("\n");
        ...
        for (idx, chat) in data.chats.iter().enumerate() {
            ...
            text.push_str(&format!("Created: {} | Model: {}\n",
                chat.created_at.format("%Y-%m-%d %H:%M:%S"),
                chat.model_used
            ));
            text.push_str("-".repeat(80).as_str());
    ...
    pub fn export_user_data(&self, data: &UserDataExport, output_dir: &Path, formats: &[&str]) -> Result<Vec<String>> {
